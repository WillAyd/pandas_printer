...
static void printByColumn(PyObject *arrList) {
    ...
    for (Py_ssize_t i = 0; i < colcount; i++) {
        ...
        do {
            printf("%lld ",
                   (long long)**dataptr); // TODO - this cast isn't correct
        } while (iternext(iter));
...
static void printByRow(PyObject *arrList, Py_ssize_t rowcount) {
    ...
    for (Py_ssize_t rowindex = 0; rowindex < rowcount; rowindex++) {
        for (Py_ssize_t i = 0; i < colcount; i++) {
            ...
            // TODO: possibly create a struct that holds iter / iternext
            // together so that we don't need to call NpyIter_GetIterNext within
            // this loop
            NpyIter_IterNextFunc *iternext = NpyIter_GetIterNext(iter, NULL);
            if (iternext == NULL) {
